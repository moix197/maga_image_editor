...
describe("TextStylePanel", () => {
  ...
  it("fires onChange with fontFamily when font family select changes", () => {
    render(<TextStylePanel node={mockNode} onChange={onChange} />);
  ...
  it("passes current fontSize as the input value", () => {
    render(<TextStylePanel node={{ ...mockNode, fontSize: 36 }} onChange={onChange} />);
    const input = screen.getByRole("spinbutton") as HTMLInputElement;
  ...
  it("passes current color as the color input value", () => {
    render(<TextStylePanel node={{ ...mockNode, color: "#123456" }} onChange={onChange} />);
    const colorInput = screen.getByLabelText("Text color") as HTMLInputElement;
  ...
  it("fires onChange with opacity when slider value changes", () => {
    render(<TextStylePanel node={{ ...mockNode, opacity: 1 }} onChange={onChange} />);
  ...
  it("reflects current opacity in slider aria-valuenow", () => {
    render(<TextStylePanel node={{ ...mockNode, opacity: 0.75 }} onChange={onChange} />);
  ...
  it("shows shadow color and blur controls only when shadow is enabled", () => {
    const { rerender } = render(
      <TextStylePanel node={{ ...mockNode, shadow: null }} onChange={onChange} />
    );
  ...
  it("fires onChange with updated shadow blur when blur slider changes", () => {
    ...
    render(<TextStylePanel node={nodeWithShadow} onChange={onChange} />);
    const rangeInputs = document.querySelectorAll('input[type="range"]');
    // With shadow enabled: index 0 = opacity, index 1 = blur
    const blurRange = rangeInputs[1] as HTMLInputElement;
