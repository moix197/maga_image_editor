...
via `updateTextNode`) and **image-overlay** nodes (geometry, via
`updateOverlayNode`). Lives in `apps/web/src/hooks/use-batch-render.ts`.
...
0. **Before the loop**, snapshot every node's template originals — for text layers
   ...
   for overlay nodes the **geometry (x/y/width/height)** — so the restore has a
...
1b. For each overlay node with an override, write its **geometry** (x/y/width/height
   only) into the live state via `updateOverlayNode(id, patch)` — a **separate
   setter** from text (`updateTextNode` and `updateOverlayNode` are both blind
   `{...n, ...patch}` spreads in `packages/editor`, so the snapshot-restore behaves
   identically across both). Overlays are **not** folded into the text path.
...
5. In a `finally`, restore every layer to its template originals — text via
   ...
   `updateOverlayNode(id, templateGeometry)` (the full x/y/width/height snapshot).
...
(`applyOverlayOverrides`) before `compositeFromElement`, or the rendered overlay
keeps the template geometry. This asymmetry (text from live DOM, image overlays
...
(x/y)/size**, **overlay geometry (x/y/width/height)**, **and** the hidden-node
opacity. If capture throws mid-loop without restoring, the template stays mutated
