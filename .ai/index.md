...
| Image-overlay aspect-lock resize | `apps/web/src/components/overlay-node-layer.tsx` (intrinsic-ratio map + corner-drag) · `overlay-controls-panel.tsx` (Size-input lock) · `batch/BatchWorkspace.tsx` (`handleNodeResize` fan-out) | Locked resize constrains to the image's intrinsic (natural) ratio, not the box's current ratio, on both the Size inputs and corner-drag; width drives height. Ratio stored in a module-scoped `Map` keyed by node id (not a persisted field). [[aspect-ratio-intrinsic-lock]] |
...
| Editor-state node creation | `apps/web/src/hooks/use-editor-state.ts` (`useEditorState`) | `addOverlayNode` returns the created node's id (not void) so callers can act on it synchronously (e.g. immediately designating it the variable slot) without racing the async `setState`. `BatchWorkspace.tsx`'s `setVariableSlotForNode(nodeId)` is the shared slot-designation core behind the variable-slot checkbox toggle. |
| Overlay-picker dialog | `apps/web/src/components/batch/OverlayPickerDialog.tsx` | Presentational, controlled multi-select thumbnail-grid dialog for reusing existing `ProjectAsset[]` overlays or falling through to upload; callbacks only (`onConfirm`, `onUploadNew`), no business logic. Not yet wired into `BatchRightPanel`. |
