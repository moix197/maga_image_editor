...
| Text-node resize + inline edit | `apps/web/src/components/text-node-layer.tsx` · `text-overlay-canvas.tsx` · `batch/BatchWorkspace.tsx` | Width-only resize (right-edge drag handle + panel Width field; height auto) and double-click inline edit (uncontrolled `contentEditable`, commit on Esc/blur). Top-left anchored (rotation pivots on center). Resize handle `stopPropagation`s so it never triggers the move handler. [[text-node-width-resize]] |
